...
> **Day-23 footnote (2026-05-14, HEAD-caa44a4 + bd 1.0.3):** the base `cycle_result` stream is written automatically — **no explicit arming required**. `gc trace status` may return `arms: null` while segments under `.gc/runtime/session-reconciler-trace/segments/YYYY/MM/DD/` continue receiving records. Arming via `gc trace start --template <T> --for <D> --level detail` is for *additional* per-template detail; the default stream that contains `cycle_result` / `cycle_start` / `cycle_input_snapshot` flows on its own. Day-19 saw zero new records after a 5-min arm and concluded "must arm to get data" — that conclusion was wrong, or has been re-fixed in HEAD-caa44a4. When checking whether tracing is working, look at segment mtimes first (`find .gc/runtime/session-reconciler-trace/segments -name '*.jsonl' -mmin -10`), not `gc trace status`. Also filter `cycle_result` records on `.fields.phase == "tick"` to exclude one-off `"startup"` cycles, which skew the distribution.
...
**Resolution (Day-15, 2026-05-13):** Premise A applied — added `formula_v2 = true` to `city.toml`'s `[daemon]` block (one line). `gc start` brought up the supervisor with all 5 control-dispatcher sessions auto-injected (visible in `gc supervisor logs` as `outcome=start_enqueued` for `control-dispatcher`, `co_auth/control-dispatcher`, `co_store/control-dispatcher`, `co_shipping/control-dispatcher`, `hello-world/control-dispatcher`). The deacon's 24h cooldown fired digest-generate 5× within the first ~5 minutes of city uptime — **all 5 produced `order.completed` events with zero `order.failed`**, breaking the prior 17/17 failure pattern. The graph-v2 workflow shape is now visible end-to-end: root beads like `cs-tc6zhp` get `gc.kind=workflow` + `gc.formula_contract=graph.v2`, four step beads (`determine-period`, `collect-data`, `generate-and-send`, `workflow-finalize`) get `gc.kind=workflow-finalize` for the terminal one, and explicit `depends_on` deps wire them into a DAG. The dog pool agent (e.g. `gastown.dog-2`) — now running under `graph-worker.md` — closed `determine-period` beads with substantive `close_reason` payloads ("Determined daily range: SINCE=…"), confirming the prompt swap is working. Bead `mc-kh9qdv` closed.

**Surprises during the flip itself:** the flag flip *worked*, but two operational surprises preceded it. (1) Both the previous-session controller (PID 787, `gc supervisor run`) and the previous-session dolt (PID 54087) had survived the prior shell crash as orphans. The orphan dolt held the `__gc_probe` lock and blocked `bd dolt start`; the orphan controller answered the port 8372 lock but hung on every `gc status` / `gc doctor` call. Both needed graceful SIGTERM before `gc start` would succeed. (2) `gc start` reported `city did not become ready under supervisor` — a false negative caused by the wake-budget queue not draining within its grace period. The supervisor logs (`gc supervisor logs`) showed 15 session candidates enqueued in wave 0, with `outcome=deferred_by_wake_budget` for several. They all came up over the following minute. Lesson for §13's reload-vs-restart triage: `gc start`'s exit status doesn't tell the whole story — read `gc supervisor logs` for ground truth, especially when wake-budget is a factor.

**Generalized §22 sub-pattern surfaced:** Day-14's "verify the *fix's* blast radius before applying" is a real §22 extension. The error message was precise ("declares graph.v2 but formula_v2 is disabled"); the one-line fix was obvious; what wasn't obvious was the two side effects (prompt swap + dispatcher injection). The §22 falsification grep (Step 1.5 in every fix-day plan) confirms the *bug's* premise. A second falsification grep — for *the fix* — would surface side effects like these. Worth promoting to "Step 1.5b" in the §22 debugging template.

# 26. Mayor under formula_v2 = true (the null-result section)

Day-16 (2026-05-13 evening) was framed as an experiment: re-run a Day-4-style full-mayor decomposition under the new `formula_v2 = true` regime, and observe what changes. The actual experiment was hijacked by reality — mayor was already in the middle of autonomous work when the day opened — and the resulting observations turned out to be the strongest evidence yet that **formula_v2 does NOT change mayor's behavior on the coordination path**. This section is mostly negative space: an enumeration of what doesn't change, plus one observation about what does.

## What happened (the worked example)

At 2026-05-13T15:09:32Z the deacon filed bead `mc-gpc5i9` ("jsonl-export SCRUB_FILTER uses non-existent column 'type'; should be 'issue_type'") after a periodic patrol cycle surfaced 5/5 `mol-dog-jsonl` failures across all dolt databases (auth, cs, hq, hw, ship). 15s later the deacon mailed mayor escalation wisp `mc-wisp-p0j2` with the **root cause already identified** (the file path, the line, the exact bad column name, and the fix). Mayor's claude session woke at 15:09:35Z.

Mayor's response: 15 events over ~3 minutes (15:09:32 → 15:12:14). Two beads created (`mc-w2emmn` nudge, `mc-wisp-vepr` reply mail), two beads closed (`mc-gpc5i9` the bug, `mc-wisp-p0j2` the wisp). Mayor applied the one-line fix directly at the live runtime path (`.gc/system/packs/maintenance/assets/scripts/jsonl-export.sh:485` — `type` → `issue_type`), ran the script to validate (`exported 5/5, records: 20009, push: ok`), closed the bug with the diff in the `close_reason`, and replied to the deacon. Then noticed a second issue (missing-`origin` was being counted as a push failure when it should be local-only), fixed it too, reset the failure counter. Drained. Total mayor cost: a short transcript and one one-line code edit.

## What didn't change (the null results)

- **Mayor's prompt template is loaded by convention from `prompt.template.md` in the agent's directory**, NOT via the formula_v2-aware default-template fallback. Day-15's prompt swap (pool-worker → graph-worker) only applies to agents whose `PromptTemplate` field is empty AND who have no convention-discovered template. Mayor has its own. Confirmed by `gc prime gastown.mayor`: first line of the rendered prompt is `# Mayor Context`, not `# Graph Worker`.
- **The five injected control-dispatcher sessions did NOT participate in mayor's work.** Mayor's window (15:09–15:13) saw zero events from any of the five (city-scope, co_auth, co_store, co_shipping, hello-world). Dispatchers fire for `gc.kind=check|fanout|retry-eval|scope-check|workflow-finalize` beads, which are produced by v2-formula compilation — not by mayor's ad-hoc bead creation.
- **Mayor created zero `gc.kind=workflow` beads.** Its beads were `chore` (nudge) and `message` (mail reply) only. The DAG-shape of v2 workflow roots only appears when a formula declared `contract = "graph.v2"` is dispatched through the periodic-orders path. Mayor's `bd bd create` style coordination produces v1 molecule beads or plain task beads — nothing the v2 contract touches.
- **Neither `mol-scoped-work` nor `mol-review-quorum` (the two coordination-flavored v2 formulas) were invoked.** Mayor didn't reach for them; they remain inert in this city despite the flag being on. They presumably get invoked only when something explicitly slings them via `gc sling` or when a pack's order list references them — and neither path fired during the day.
- **No cross-rig handoff occurred.** The fix was a single-rig HQ change. Day-4's `mc-wjos2g` cross-rig convoy gap was not exercised; v2 doesn't change that path either way.

## What did change (the one observation)

The fix mayor applied lives in `.gc/system/packs/maintenance/assets/scripts/jsonl-export.sh:485`. That path is **gitignored at the HQ city level** — `.gc/` is in the city's `.gitignore`. So mayor's "uncommitted change" can't actually be committed to the HQ repo: there's nothing to commit. Mayor noted this in its reply ("No tracked git changes in `/Users/rfvitis/my-city` because `.gc/` is ignored.") and cross-checked the upstream submodule pointer to confirm the canonical fix already lives there (`study/gascity-src` @ `4819165`, the PR #2037-era pointer with this exact column-name fix in the source-of-truth tree).

This is **not** a v2 finding per se, but it's a Day-16 surprise: the live runtime path is editable but not committable from the city. Operationally, mayor's fix only survives until the maintenance pack gets re-installed (e.g., `gc pack refresh`, or a city re-init). For permanent fixes, the work has to land upstream — which is where PR #2037's playbook (§24) becomes load-bearing.

## When to expect v2 to actually matter

The honest summary: **v2 shows up only in the periodic-orders path** (Day-15's digest-generate worked example). For mayor's coordination, ad-hoc bead creation, and human-prompted incident response, v2 is invisible. The pool agent layer (dogs, dispatched on-demand workers) sees v2 every time a v2 formula dispatches. Everywhere else, it's a no-op.

This means **decisions about when to "use v2" are not user-facing.** The user doesn't write graph.v2-shaped molecules by hand; the pack maintainers do. The flag flip is a one-time bootstrap step, not a workflow choice. Once it's on, the city operates the same way at the user level — periodic orders just stop silently failing.

## What this means for the §17/§22 mental model
